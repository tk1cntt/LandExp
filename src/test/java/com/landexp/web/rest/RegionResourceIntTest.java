...
public class RegionResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllRegionsByDistrictsIsEqualToSomething() throws Exception {
        // Initialize the database
        District districts = DistrictResourceIntTest.createEntity(em);
        em.persist(districts);
        em.flush();
        region.addDistricts(districts);
        regionRepository.saveAndFlush(region);
        Long districtsId = districts.getId();

        // Get all the regionList where districts equals to districtsId
        defaultRegionShouldBeFound("districtsId.equals=" + districtsId);

        // Get all the regionList where districts equals to districtsId + 1
        defaultRegionShouldNotBeFound("districtsId.equals=" + (districtsId + 1));
    }
