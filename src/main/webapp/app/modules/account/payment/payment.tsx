  ...
  updatePaymentInfo() {
    return (
      <Row>
        ...
        <Col md="6" style={{ color: 'red' }}>{this.props.payment.code}</Col>
        <Col md="6">Phí đăng tin</Col>
        <Col md="6" style={{ color: 'red' }}>{new Intl.NumberFormat().format(this.props.payment.money)} VNĐ</Col>
      </Row>
    );
  }
  ...
  updateTransferInfo() {
    return (
      <Row style={{ padding: 20 }}>
        ...
        <Col md="6" style={{ fontWeight: 'bold' }}>Công ty TNHH Abuma</Col>
        <Col md="6">Số tài khoản:</Col>
        <Col md="6" style={{ fontWeight: 'bold', color: 'blue' }}>0123456789</Col>
        <Col md="6">Ngân hàng:</Col>
        <Col md="6">Á Châu (ACB)</Col>
        <Col md="6">Số tiền:</Col>
        <Col md="6" style={{ color: 'red' }}>{new Intl.NumberFormat().format(this.props.payment.money)} VNĐ</Col>
        <Col md="6">Nội dung chuyển khoản:</Col>
        <Col md="6" style={{ fontWeight: 'bold' }}>TTT {this.props.payment.code}</Col>
      </Row>
    );
  }
        ...
        <Col md="8">
          <div style={{ marginTop: 10, background: '#ECECEC', height: '100%', padding: '5px' }}>
            <Card title="Thanh toán phí đăng tin" bordered={false} style={{ width: '100%', height: '100%' }}>
              <div><h6>Bạn vui lòng chọn hình thức thanh toán phí đăng tin dưới đây.</h6></div>
              <div>
                <RadioGroup onChange={this.onChange} value={this.state.value}>
                  <Radio style={radioStyle} value={1}>Thanh toán chuyển khoản</Radio>
                  <Radio style={radioStyle} value={2}>Thanh toán online</Radio>
                </RadioGroup>
              </div>
