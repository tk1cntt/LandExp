...
export const ACTION_TYPES = {
  SEARCH_DISTRICTS: 'district/SEARCH_DISTRICTS',
...
export default (state: DistrictState = initialState, action): DistrictState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_DISTRICTS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_DISTRICTS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_DISTRICTS):
      return {
        ...state,
        loading: false,
...
const apiSearchUrl = SERVER_API_URL + '/api/_search/districts';

// Actions

export const getSearchEntities: ICrudSearchAction<IDistrict> = query => ({
  type: ACTION_TYPES.SEARCH_DISTRICTS,
  payload: client.get<IDistrict>(`${apiSearchUrl}?query=` + query)
});
