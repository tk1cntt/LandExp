...
const apiSearchUrl = SERVER_API_URL + '/api/_search/articles';

// Actions

export const getSearchEntities: ICrudSearchAction<IArticle> = query => ({
  type: ACTION_TYPES.SEARCH_ARTICLES,
  payload: client.get<IArticle>(`${apiSearchUrl}?query=` + query)
});
